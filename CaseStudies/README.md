...
ten application studies discussed in Section 3 of the paper. Each subfolder corresponds to one
...
study's `reproduce.py` (add `--smoke` for a fast check) or `run_all.py --smoke` to exercise all ten.
...
| 4 | [administrative-portal-message-triage](administrative-portal-message-triage/) | 3.2.1 | Portal-message triage (local Qwen2.5-0.5B; DistilBERT heads) |
| 5 | [postpartum-severity-triage](postpartum-severity-triage/) | 3.2.2 | Postpartum severity triage (GPT-4o-mini; BioBERT cascade / Bi-LSTM / GPT-4o-mini) |
| 6 | [oncology-distress-classification](oncology-distress-classification/) | 3.2.3 | Psychosocial response + distress (Gemma2-27B; dual-LLM judge; DistilBERT) |
| 7 | [medication-question-risk-classification](medication-question-risk-classification/) | 3.2.4 | Medication-question risk triage (GPT-4.1 augmentation; BioBERT / BlueBERT) |
| 8 | [home-care-status-detection](home-care-status-detection/) | 3.2.5 | Home-care status detection (GPT; LightGBM fusion) |
| 9 | [ems-report-routing](ems-report-routing/) | 3.3.1 | EMS report routing (GPT-4.1-mini + ASR noise; BioClinicalBERT) |
| 10 | [sbar-completeness-checking](sbar-completeness-checking/) | 3.3.2 | SBAR completeness checking (OpenAI batch; BioClinicalBERT; hierarchical) |
